...
import { pushState, redoChange, undoChange, StateHistory } from "./stateHistory";
...
describe("pushState", () => {
  ...
  it("clears the redo states", () => {
    const history: StateHistory<number> = {
      currentState: 0,
      redoStates: [5, 6, 7],
      undoStates: [],
    };
  ...
  it("enforces size bounds on the undo states", () => {
    ...
    expect(pushState(history, 4, 3)).toEqual({
      currentState: 4,
      redoStates: [],
      undoStates: [1, 2, 3],
    });
  ...
  it("does not truncate if the size bound is not exceeded", () => {
    ...
    expect(pushState(history, 4, 10)).toEqual({
      currentState: 4,
      redoStates: [],
      undoStates: [0, 1, 2, 3],
    });
