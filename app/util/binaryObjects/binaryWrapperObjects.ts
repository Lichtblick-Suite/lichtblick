...
const printDeepParseField = (
  ...
  if (isArray) {
    ...
    if (type === "int8") {
      ...
      ret.push(
        `for (let $ptr = ${name}$from, $i = 0; $i < ${name}$length; $ptr += ${elementSize}) {`,
      );
...
$context.associateDatatypes(${friendlyTypeName(typeName)}, [$typesByName, ${JSON.stringify(
    typeName,
  )}])`;
};
...
const getGetClassForView = memoize((typesByName: RosDatatypes, typeName: string) => {
  ...
  return Function(
    ...
    "$bigString",
    "$typesByName",
    printGetClassForView(typesByName, typeName),
  );
