...
describe("getObjects", () => {
  it("can compile everything", () => {
    ...
    const bigString = "";
    expect(
      getObjects(definitions, "fake_msgs/ContainsEverything", intArray.buffer, bigString, []),
    ).toEqual([]);
  });
  ...
  it("can make classes with constants", () => {
    const intArray = new Int32Array([]);
    const bigString = "";
    const hasConstant = cast<HasConstant>(
      getObject(definitions, "fake_msgs/HasConstant", intArray.buffer, bigString),
    );
  ...
  it("can handle byte arrays", () => {
    ...
    expect(deepParse(hasByteArray)).toEqual({
      byte_array: new Uint8Array([0xef, 0xbe, 0xad, 0xde]),
    });
  });
  ...
  it("can handle JSON fields", () => {
    ...
    const hasJson = cast<HasJson>(
      getObject(definitions, "fake_msgs/HasJson", intArray.buffer, bigString),
    );
  ...
  it("can handle 64-bit integers", () => {
    ...
    const hasInt64s = cast<HasInt64s>(
      getObject(definitions, "fake_msgs/HasInt64s", intArray.buffer, ""),
    );
    expect(deepParse(hasInt64s)).toEqual({ i64: -1, u64: 1 });
  });

  it("throws on access for out-of-range 64-bit integers", () => {
    console.assert = (statement: unknown, message?: string) => {
      if (!statement) {
  ...
  it("doesn't crash when given arrays of empty messages", () => {
    ...
    const hasArr1 = cast<HasArrayOfEmpties>(
      getObject(definitions, "fake_msgs/HasArrayOfEmpties", intArray.buffer, ""),
    );
    expect(deepParse(hasArr1)).toEqual({ arr: [] });

    intArray[0] = 3;
    const hasArr2 = cast<HasArrayOfEmpties>(
      getObject(definitions, "fake_msgs/HasArrayOfEmpties", intArray.buffer, ""),
    );
...
describe("wrapJsObject", () => {
  ...
  it("can handle primitive arrays", () => {
    ...
    const wrapped = cast<HasComplexAndArray>(
      wrapJsObject(definitions, "fake_msgs/HasComplexAndArray", js),
    );
  ...
  it("can handle complex arrays", () => {
    ...
    const hasComplexArray = cast<HasComplexArray>(
      wrapJsObject(definitions, "fake_msgs/HasComplexArray", js),
    );
    const complexArray = hasComplexArray.complexArray();
    expect(complexArray.length()).toBe(1);
    const complex = complexArray.get(0);
...
describe("accessor methods", () => {
  ...
  describe("getFieldFromPath", () => {
    ...
    it.each([true, false])("handles a complex object", (useBobjects) => {
      ...
      const obj = useBobjects ? wrapJsObject(definitions, "fake_msgs/ContainsEverything", js) : js;
      expect(getFieldFromPath(obj, ["third", "complexArray", 0, "header", "stamp", "nsec"])).toBe(
        2,
      );
      expect(getFieldFromPath(obj, ["third", "complexArray", 1, "header", "stamp", "nsec"])).toBe(
        456,
      );
      expect(
        getFieldFromPath(obj, ["third", "complexArray", 1, "header", "stamp", "INVALID"]),
      ).toBeUndefined();
