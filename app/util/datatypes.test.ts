...
describe("getTransitiveSubsetForDatatypes ", () => {
  ...
  it("works for type with no children", () => {
    expect(
      Object.keys(getTransitiveSubsetForDatatypes(basicDatatypes, ["std_msgs/Header"])),
    ).toEqual(["std_msgs/Header"]);
  });

  it("works for types with children", () => {
    expect(
      new Set(Object.keys(getTransitiveSubsetForDatatypes(basicDatatypes, ["geometry_msgs/Pose"]))),
    ).toEqual(new Set(["geometry_msgs/Pose", "geometry_msgs/Point", "geometry_msgs/Quaternion"]));
  });
  ...
  it("works when some input types have children and others don't", () => {
    expect(
      new Set(
        Object.keys(
          getTransitiveSubsetForDatatypes(basicDatatypes, [
            "geometry_msgs/Pose",
            "std_msgs/Header",
          ]),
        ),
