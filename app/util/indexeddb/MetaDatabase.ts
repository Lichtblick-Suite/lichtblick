...
function tryDelete(databaseName: string): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    deleteRequest.onblocked = () => {
      log.info(
        `Could not delete ${databaseName} - request blocked.  This database is likey open in another tab.`,
      );
      done(false);
    };
  });
...
export async function updateMetaDatabases(
  ...
  try {
    ...
    const old = all.sort((a: any, b: any) => b.lastAccess - a.lastAccess).slice(maxDatabases);
    // go through the list of old databases and try to delete each of them
    const promises = old.map(async (lastUsed: any) => {
