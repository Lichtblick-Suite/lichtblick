...
export function isWebvizNodeTopic(topic: string) {
  return (
    topic.startsWith(WEBVIZ_NODE_PREFIX) ||
    topic.startsWith(`${SECOND_SOURCE_PREFIX}${WEBVIZ_NODE_PREFIX}`)
  );
}
...
function validateDatatypes({ output, datatypes }: NodeDefinition<any>) {
...
export function validateNodeDefinitions(nodeDefinitions: NodeDefinition<any>[]): void {
...
function applyNodeToMessage<State>(
  ...
  const { messages: unfilteredMessages, state } = nodeDefinition.callback({
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    nodeDefinitions.forEach((nodeDefinition) => {
      ...
      if (nodeDefinition.inputs.includes(message.topic) && formatMatches) {
        ...
        let nodeResult: { state: any; messages: Message[] } = {
          messages: [],
          state: previousState,
        };
          ...
          nodeDefinition.format === "parsedMessages"
            ...
            : nodeResult.messages.map((nodeMessage) => {
                ...
                return {
                  ...
                  message: wrapJsObject(
                    datatypes,
                    nodeDefinition.output.datatype,
                    nodeMessage.message,
                  ),
                };
              });
...
export function getNodeSubscriptions(nodeDefinitions: NodeDefinition<any>[]): SubscribePayload[] {
  const allActiveNodes = uniq(
    flatten(
      nodeDefinitions.map((rootNode) =>
        getDependentNodeDefinitions(
          nodeDefinitions.map((def) => def),
          rootNode,
        ),
      ),
    ),
...
export function getDefaultNodeStates(nodeDefinitions: NodeDefinition<any>[]): NodeStates {
