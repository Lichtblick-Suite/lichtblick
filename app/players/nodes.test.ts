...
describe("nodes", () => {
  ...
  describe("applyNodesToMessages", () => {
    ...
    it("returns properly wrapped bobjects irrespective of how the node returns them", () => {
        ...
        callback() {
          return {
            ...
            messages: [
              ...
              makeNodeMessage("/webviz/foo", rawReturnedBobject1), // Returns a bobject
              makeNodeMessage(
                "/webviz/foo",
                wrapJsObject(basicDatatypes, "std_msgs/Header", rawReturnedBobject2),
              ),
            ],
          };
        },
      ...
      expect(
        output.messages
          .filter(({ topic }) => topic === "/webviz/foo")
          .map(({ message }) => deepParse(message)),
