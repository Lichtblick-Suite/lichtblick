...
const testMessageDefinition = {
  fields: [
    ...
    { type: "int8", name: "callCount" },
  ],
};

const node: NodeDefinition<any> = {
...
const bobjectNode: NodeDefinition<any> = {
  inputs: ["/input/foo"],
...
describe("NodePlayer", () => {
  ...
  it("will throw if the output datatype isn't present", () => {
    ...
    expect(
      () =>
        new NodePlayer(fakePlayer, [
          { ...node, output: { name: "/webviz/test", datatype: "unknown_msgs/T" } },
        ]),
  ...
  it("includes a node if one of the input topics is available", () => {
    ...
    const nodePlayer = new NodePlayer(fakePlayer, [
      { ...node, inputs: ["/input/foo", "/unavailable_topic"] },
    ]);
    const messages: PlayerState[] = [];
    nodePlayer.setListener(async (playerState) => {
      messages.push(playerState);
  ...
  describe("bobjects", () => {
    ...
    it("does not emit twice the number of messages if subscribed to twice", async () => {
      ...
      expect(parsedMessages.filter(({ topic }) => topic === "/webviz/bobject_test").length).toEqual(
        2,
      );
