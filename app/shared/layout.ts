...
export function getLayoutNameAndVersion(
...
export function getLayoutFolder(layoutName?: string | null | undefined): string {
...
export function getPanelStateHash(panels: PanelsState): string {
  ...
  const hash = require("crypto").createHash("sha256").update(JSON.stringify(panels)).digest("hex");
...
export function getFolderIdAndLayoutNameFromLayoutId(
  layoutId: string,
): { folderId: string; layoutName: string } {
