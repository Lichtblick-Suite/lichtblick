...
export default class IdbCacheWriterDataProvider implements DataProvider {
  ...
  _updateState() {
    ...
    if (this._totalNs === undefined) {
      throw new Error("Invariant violation: IdbCacheWriterDataProvider totalNs is undefined");
    }
  ...
  async _setConnection(range: Range) {
    ...
    if (!this._db || !this._startTime) {
      sendNotification("IdbCacheWriter not initialized", "", "app", "error");
      return;
    }
    ...
    for (;;) {
      ...
      const topics = currentConnection.topics.filter((topic: string) => {
        const missingRangeForTopic = missingRanges(
          currentRange,
          this._rangesByTopic[topic] || [],
        )[0];
