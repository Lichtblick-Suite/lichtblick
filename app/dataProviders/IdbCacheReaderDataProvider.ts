...
export default class IdbCacheReaderDataProvider implements DataProvider {
  ...
  async getMessages(
    ...
    if (!this._startTime || !this._db) {
      throw new Error("IdbCacheReaderDataProvider not initialized");
    }
    const range = {
    ...
    store
      ...
      .iterateKeyCursor(IDBKeyRange.bound(range.start, range.end - 1), (cursor) => {
        if (!cursor) {
          return;
        }
