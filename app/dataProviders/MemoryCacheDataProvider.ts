    ...
    for (let blockIndex = startIndex; blockIndex !== endIndex; blockIndex += increment) {
      ...
      if (cacheSizeInBytes > maxCacheSizeInBytes) {
        const newRecentRangesExcludingBadEvictionRange = [
          ...
          increment > 0
            ? { start: 0, end: blockIndex + 1 }
            : { start: blockIndex, end: blockRange.end },
        ];
...
export default class MemoryCacheDataProvider implements DataProvider {
  ...
  _currentConnection:
    | { id: string; topics: string[]; remainingBlockRange: Range }
    | null
    | undefined;
  ...
  async getMessages(
    ...
    if (!this._startTime || this._memCacheBlockSizeNs === undefined) {
  ...
  async _setConnection(blockRange: Range): Promise<boolean> {
    ...
    // eslint-disable-next-line no-constant-condition
    while (true) {
      const currentConnection: any = this._currentConnection;
      ...
      for (const bobjectMessage of bobjects || []) {
        // @ts-expect-error .push does not exist on the readonly array
        // the array is readonly because of the messagesByTopic assignment which uses spread
        // to copy the fields into the messagesByTopic object but each fields is a readonly array
        // This is likely pointing to un-intended behavior or if the intent is to change the original
        // array then we need to rework this.
