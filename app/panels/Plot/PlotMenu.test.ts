...
const {
  ...
  datasets: trackedObjectsDatasets,
...
describe("PlotMenu", () => {
  ...
  it("Multiple topics", () => {
    expect(
      getCSVData(datasetsMultipleTopics, tooltipsMultipleTopics, "timestamp"),
    ).toMatchSnapshot();
  });
  ...
  it("Multiple topics with different header.stamp", () => {
    expect(
      getCSVData(datasetsMultipleTopics, tooltipsDiffTimestamp, "timestamp"),
    ).toMatchSnapshot();
  });

  it("works with data directly from getDatasetsAndTooltips", () => {
    expect(
      getCSVData(trackedObjectsDatasets, trackedObjectsTooltips, "timestamp"),
    ).toMatchSnapshot();
  });
});
